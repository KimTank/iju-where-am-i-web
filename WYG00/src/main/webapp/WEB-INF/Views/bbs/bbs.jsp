...
<body class="index-page sidebar-collapse">
	...
	<div class="page-header" filter-color="orange">
		<div class="page-header-image" style="background-image: url('./assets/img/bbs1.jpg');"></div>
		<!-- 게시판부분 -->
		<!-- 게시판 java -->
		<%
		//게시판 페이지 알수있게
		...
		<div class="container">
			<div class="row">
				<table class="table table-striped" style="text-align: center; border: 1px solid #F5A9D0">
					...
					<tbody>
						...
						%>
							...
							<td>
								<a href="t2view?bbsId=<%=list.get(i).getBbsId()%>"> <%=list.get(i).getBbsTitle().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;")
				...
				if (pageNumber != 1) {
			%>
				<a href="t1bbs?pageNumber=<%=pageNumber - 1%>" class="btn btn-success btn-arraw-left">이전</a>
				<%
				}
				if (bbsDAO.nextPage(pageNumber + 1)) {
			%>
				<a href="t1bbs?pageNumber=<%=pageNumber + 1%>" class="btn btn-success btn-arraw-left">다음</a>
				<%
				}
			%>
				<a href="t2write" class="btn btn-primary pull-right">글쓰기</a>
			</div>
		</div>
